...
function getLoginOptions(args = LOGIN_ARGS) {
...
function setStartupEnabled(enabled) {
  if (process.platform === "linux") {
    writeGeneralPrefs({ ...readGeneralPrefs(), startAtLogin: enabled });
...
function iconPath() {
  // A single canonical Lumi identity is used for the window, tray and packaged app.
  // Do not fall back to legacy ICO artwork: the approved PNG is the source of truth.
  return app.isPackaged
    ? path.join(process.resourcesPath, "static", "favicon-256.png")
    : path.resolve(__dirname, "..", "static", "favicon-256.png");
}

function generalPrefsPath() {
  return path.join(app.getPath("userData"), "LUMIDM-prefs.json");
}
...
function readGeneralPrefs() {
  return readJson(generalPrefsPath(), {});
}

function writeGeneralPrefs(value) {
  writeJson(generalPrefsPath(), value);
}
...
function readDesktopPrefs() {
  return readJson(desktopPrefsPath(), defaultDesktopPrefs());
}
...
function createWidget() {
  ...
  void widgetWindow.loadFile(path.join(__dirname, "widget-approved.html"));
...
function showWidget() {
  const settings = readDesktopPrefs();
  if (settings.visible === false || setupWindow || mainWindowIsVisible()) {
    hideWidget();
    return;
  }
...
async function showSetupPopup(task) {
  ...
  setupWindow = new BrowserWindow({
    ...
    focusable: true,
    show: false,
    title: "Lumi download confirmation",
    icon: iconPath(),
    webPreferences: {
      contextIsolation: true,
...
async function scanPendingSetups() {
  ...
  try {
    ...
    const pending = (result.downloads || []).find(task => (
      task.status === "browser_pending" && task.metadata?.browser_handoff_id
    ));
...
async function pollTasks() {
  try {
    ...
    if (tray) {
      tray.setToolTip(lastActive
        ? `Lumi DM · ↓ ${formatSpeed(lastSpeed)} · ${lastActive} active`
        : "Lumi DM");
    }
    ...
    if (!baselineReady) {
      ...
      for (const id of taskBaseline.keys()) {
        if (!liveIds.has(id)) taskBaseline.delete(id);
      }
    }
...
function rebuildTrayMenu() {
  ...
  tray.setContextMenu(Menu.buildFromTemplate([
    ...
    { label: "Show download widget", click: showWidget },
    { label: "Hide download widget", click: hideWidget },
    { type: "separator" },
    {
...
function registerIpc() {
  ...
  ipcMain.handle("v5-setup-confirm", async (event, value) => {
    ...
    const result = await requestJson(
      "POST",
      `/api/v5/browser/handoffs/${encodeURIComponent(data.handoffId)}/confirm`,
      value || {},
    );
    closeSetup(true);
    return result;
  ...
  ipcMain.handle("v5-setup-browser", async event => {
    ...
    const result = await requestJson(
      "POST",
      `/api/v5/browser/handoffs/${encodeURIComponent(data.handoffId)}/browser`,
      {},
    );
    closeSetup(true);
    return result;
  ...
  ipcMain.handle("v5-setup-cancel", async event => {
    ...
    const result = await requestJson(
      "POST",
      `/api/v5/browser/handoffs/${encodeURIComponent(data.handoffId)}/cancel`,
      {},
    );
    closeSetup(true);
    return result;

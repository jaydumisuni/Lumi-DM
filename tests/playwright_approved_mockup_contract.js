...
async function main(){
 ...
 const go=async view=>{
  ...
  if(nested.has(view)){
   ...
   await p.click(`.nav-item[data-view="${view}"]`);
  }
  await p.locator(`#view-${view}.active`).waitFor({state:"visible"});
 };
 ...
 await go("firmware"); await p.waitForTimeout(150); assert(await p.locator("#view-firmware .approved-firmware-page").count()===1,"Mobile Firmware page mismatch"); assert(await p.locator("#view-firmware .approved-platform-tabs button").count()===2,"Mobile Firmware platform tabs mismatch"); assert(await p.locator("#view-firmware .approved-firmware-table").count()===1,"Mobile Firmware table mismatch");
 await go("operating_systems"); await p.waitForTimeout(150); assert(await p.locator("#view-operating_systems .approved-os-page").count()===1,"Operating Systems page mismatch"); assert(await p.locator("#view-operating_systems .approved-platform-tabs button").count()===3,"Operating Systems platform tabs mismatch"); assert(await p.locator("#view-operating_systems .approved-os-table").count()===1,"Operating Systems table mismatch");

...
def lumi_server(data_dir: Path, download_dir: Path, temp_dir: Path):
    ...
    try:
        ...
        if process.returncode not in (0, -15, -9):
            ...
            raise AssertionError(f"Lumi server exited {process.returncode}:\n{output[-8000:]}")
...
def wait_for_file(path: Path, expected: bytes, timeout: float = 20.0) -> None:
    ...
    raise AssertionError(
        f"downloaded file was not completed correctly: exists={path.exists()} bytes={len(current)}"
    )


def click_view(page, view: str) -> None:
    page.locator(f'.nav-item[data-view="{view}"]').click()
    expect(page.locator(f"#view-{view}")) .to_have_class(lambda value: "active" in value.split())
    ...
    with fixture_server(fixture_dir) as fixture_port, lumi_server(data_dir, download_dir, temp_dir):
        with sync_playwright() as playwright:
            ...
            page.wait_for_function(
                "document.documentElement.dataset.lumiInteractionContract === 'ready'",
                timeout=12_000,
            )

            # Every top-level route must physically switch the visible workspace.
            for view in (
                "overview",
                "downloads",
                "unfinished",
            ...
            # Technician is a real dropdown, not a decorative heading.
            ...
            expect(page.locator("#view-firmware")).to_have_class(lambda value: "active" in value.split())
            ...
            expect(page.locator("#view-operating_systems")).to_have_class(lambda value: "active" in value.split())
            page.locator("#view-operating_systems .os-catalogue-shell").wait_for(state="visible", timeout=10_000)
            expect(technician).to_have_attribute("aria-expanded", "true")

            # Settings and each settings tab are interactive through the same delegated shell.
            click_view(page, "overview")
            page.locator('[data-main-view="settings"]').first.click()
            expect(page.locator("#view-settings")).to_have_class(lambda value: "active" in value.split())
            page.locator('[data-main-settings-tab="storage"]').click()
            expect(page.locator('[data-main-settings-section="storage"]')).to_have_class(
                lambda value: "active" in value.split()
            )
            page.locator('[data-main-settings-tab="security"]').click()
            expect(page.locator('[data-main-settings-section="security"]')).to_have_class(
                lambda value: "active" in value.split()
            )

            # Queue creation and the approved ellipsis now have actual actions behind them.
            ...
            page.locator("#queue-form").locator('button[type="submit"]').click()
            ...
            expect(queue_card.locator('.lumi-card-contract-menu [data-contract-forward="toggle-queue"]')).to_be_visible()
            page.keyboard.press("Escape")

            # Category creation proves modal/form routing and dynamic re-rendering.
            ...
            page.locator("#category-form").locator('button[type="submit"]').click()
            ...
            # LinkGrabber analyses a real local page and exposes the discovered file.
            click_view(page, "grabber")
            grab_form = page.locator('#view-grabber form[data-form="grabber"]')
            grab_form.locator('textarea[name="url"]').fill(
                f"http://127.0.0.1:{fixture_port}/links.html"
            )
            grab_form.locator('button[type="submit"]').click()
            ...
            # New Download is exercised across every source tab before a real local HTTP transfer.
            ...
            for source, selector in tab_contracts.items():
            ...
            values = connections.locator("option").evaluate_all(
                "options => options.map(option => option.value)"
            )
            connections.select_option("1" if "1" in values else values[0])
            direct.locator('button[type="submit"]').click()

            expect(page.locator("#new-modal")).to_be_hidden(timeout=15_000)
            expect(page.locator("#view-downloads")).to_have_class(
                lambda value: "active" in value.split()
            )
            ...
            # No renderer exception is acceptable during the complete interaction sweep.

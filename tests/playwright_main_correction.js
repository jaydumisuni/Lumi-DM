...
async function main() {
  ...
  await page.route("**/api/v5/firmware/catalogue", async route => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        warning: "Verify the exact model before flashing.",
        brands: ["Samsung", "Apple", "Google Pixel"],
        providers: [
          { id: "samsung", name: "Samsung Support", group: "Official OS", brands: ["Samsung"] },
          { id: "android", name: "AndroidFileHost", group: "Community mirrors", brands: ["Android"] },
        ],
      }),
    });
  });
  await page.route(/\/api\/v5\/firmware\/devices\?/, async route => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        devices: [
          { id: "SM-S918B", name: "Galaxy S23 Ultra", model: "SM-S918B", codename: "dm3q", provider: "samsung", brand: "Samsung" },
          { id: "SM-S918U", name: "Galaxy S23 Ultra US", model: "SM-S918U", codename: "dm3q", provider: "samsung", brand: "Samsung" },
        ],
      }),
    });
  });
  await page.route(/\/api\/v5\/firmware\/search\?/, async route => {
  ...
  const geometry = await page.evaluate(() => {
    ...
    return {
      ...
      overviewBottom: Math.round(overview.getBoundingClientRect().bottom),
  ...
  for (const action of ["minimize", "maximize", "close"]) {
    await page.click(`[data-window-action="${action}"]`);
  }
  ...
  assert(firmwareOrder[0].startsWith("Brand"), "Firmware selector does not start with Brand");
  assert(firmwareOrder[1].startsWith("Model"), "Firmware selector does not place Model second");
  assert(firmwareOrder[2].startsWith("Source"), "Firmware selector does not place Source after Model");
  assert(firmwareOrder[3].startsWith("Channel"), "Firmware selector does not place Channel after Source");

  const model = page.locator("#lumi-firmware-model");
  const source = page.locator("#lumi-firmware-source");
  assert(await model.isDisabled(), "Model is selectable before Brand");
  assert(await source.isDisabled(), "Source is selectable before Model");
  await page.selectOption("#lumi-firmware-brand", "Samsung");
  await page.waitForFunction(() => !document.getElementById("lumi-firmware-model").disabled);
  await model.fill("SM-S918B");
  await page.waitForFunction(() => !document.getElementById("lumi-firmware-source").disabled);
  ...
  await page.locator("#view-overview.active").waitFor({ state: "visible" });
  await page.click("#new-download-btn");
  ...
  const finalState = await page.evaluate(() => ({
    interactionControls: document.documentElement.dataset.lumiInteractionControls,
    interactionBlocked: document.documentElement.dataset.lumiInteractionBlocked,
    stage0Events: (window.__lumiStage0 || []).length,
  }));

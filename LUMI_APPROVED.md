...
10. Keep **one canonical Lumi checkout per machine**. On KRATOS the canonical checkout is `/home/kratos/projects/Lumi-DM`; other clones/worktrees are temporary only.
11. GitHub commit history is the machine-to-machine continuation authority. ATHENA and KRATOS sync by fetch/pull/push; do not copy whole project folders between machines.
12. Before work begins, fetch the remote, verify the intended branch/upstream and inspect the working tree. Do not continue substantial work from a stale checkout while another checkout is ahead.
13. Temporary worktrees, proof clones, virtual environments, screenshots, runtime databases and Builder outputs must live in ignored/cache/temp locations. When the work is integrated or preserved in Git, remove the temporary local copy.
14. If a temporary checkout contains unique unfinished work, preserve it first as a clearly non-authoritative Git branch or commit; then remove the duplicate local folder. Never delete unique work merely to make the workspace look clean.
15. A completed Lumi change leaves the canonical checkout clean and the authoritative remote SHA verified.
